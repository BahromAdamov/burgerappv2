...
const OrderHistory: React.FC<OrderHistoryProps> = ({ phone }) => {
  ...
  if (loading) return (
    <div className="py-24 flex flex-col items-center text-gray-300">
      ...
      <span className="text-[11px] font-black uppercase tracking-widest">{t('loadingHistory')}</span>
    </div>
  );
  ...
  if (error) return (
    <div className="py-32 flex flex-col items-center text-gray-300 px-10 text-center">
      ...
      <p className="font-black uppercase text-[10px] tracking-widest text-gray-400">{t('connectionError')}</p>
      <button onClick={fetchHistory} className="mt-4 px-6 py-2.5 bg-black text-white rounded-xl text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-lg">{t('tryAgain')}</button>
    </div>
  );
  ...
  if (orders.length === 0) return (
    <div className="py-32 flex flex-col items-center text-gray-300 px-10 text-center">
      ...
      <p className="font-black uppercase text-[11px] tracking-widest text-gray-400">{t('noOrders')}</p>
      <p className="text-[9px] font-bold mt-2 text-gray-300 uppercase leading-relaxed max-w-[200px]">{t('firstOrderHint')}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-6 pb-10 animate-in fade-in duration-500">
      <div className="flex items-center justify-between px-2">
         <h2 className="text-2xl font-black uppercase tracking-tighter italic">{t('history')}</h2>
         <button 
      ...
      <div className="space-y-5">
        {orders.map((order) => {
          const statusLabels: Record<string, string> = {
            pending: t('pending'),
            cooking: t('cooking'),
            ready_pickup: t('ready'),
            ready_delivery: t('packed'),
            on_way: t('onWay'),
            completed: t('completedStatus'),
            cancelled: t('cancelledStatus'),
          };
          const statusInfo = { label: statusLabels[order.status] || order.status, color: statusColors[order.status] || '#000' };
          let dateStr = '', timeStr = '';
          try {
             const dateObj = new Date(order.date);
             dateStr = dateObj.toLocaleDateString(language === 'uz' ? 'uz-UZ' : 'ru-RU', { day: 'numeric', month: 'short' });
             timeStr = dateObj.toLocaleTimeString(language === 'uz' ? 'uz-UZ' : 'ru-RU', { hour: '2-digit', minute: '2-digit' });
          } catch {
            // ignore
          }
            ...
            <div key={order.orderId} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col gap-5 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-xs font-black uppercase tracking-tight">{t('order')} {order.orderId.slice(-4)}</h4>
              ...
              <div className="bg-gray-50/50 rounded-[1.5rem] p-4 space-y-3">
                <div className="space-y-1">
                  ...
                  )) : <span className="text-[10px] font-bold text-gray-400 uppercase italic">{t('noData')}</span>}
                </div>
                ...
                <div className="pt-3 flex justify-between items-center border-t border-gray-100">
                  ...
                  <span className="text-sm font-black text-black tracking-tight">{order.total ? order.total.toLocaleString() : 0} {t('sum').toUpperCase()}</span>
                </div>
              </div>
            </div>
          );
