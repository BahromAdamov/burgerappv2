...
const FloatingStatus: React.FC<FloatingStatusProps> = ({ orderId, onClose, orderType, restaurantPhone, isMinimized, onToggleMinimize }) => {
  ...
  const steps = orderType === 'delivery' 
    ? [
        { id: 'pending', label: 'Принят', icon: <CheckCircle2 className="w-3.5 h-3.5" /> },
        { id: 'cooking', label: 'Готовится', icon: <ChefHat className="w-3.5 h-3.5" /> },
        { id: 'on_way', label: 'В пути', icon: <Truck className="w-3.5 h-3.5" /> }
      ]
    : [
        { id: 'pending', label: 'Принят', icon: <CheckCircle2 className="w-3.5 h-3.5" /> },
        { id: 'cooking', label: 'Готовится', icon: <ChefHat className="w-3.5 h-3.5" /> },
        { id: 'ready_pickup', label: 'Готов', icon: <Package className="w-3.5 h-3.5" /> }
      ];
        ...
        <div className="relative pointer-events-auto">
          <div className="bg-black/95 backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-4 shadow-2xl flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">ЗАКАЗ {orderId.slice(-4)}</p>
          ...
          >
            ...
            <div className="pr-3 flex items-center gap-2">
              <span className="text-[10px] font-black text-white uppercase tracking-wider">
                {steps[currentIdx]?.label || 'В обработке'}
              </span>
          ...
          <div className="bg-black/95 backdrop-blur-2xl border border-white/10 rounded-[3rem] p-6 shadow-2xl flex flex-col gap-6 overflow-hidden relative">
            ...
            <div className="flex items-center justify-between relative">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[11px] font-black text-white uppercase tracking-[0.25em]">ЗАКАЗ {orderId.slice(-4)}</span>
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="text-center py-2">
              <h3 className="text-2xl font-black text-white uppercase tracking-tight mb-1">
                {steps[currentIdx]?.label || 'Обработка'}
              </h3>
              <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">
                {status === 'pending' ? 'Ваш заказ принят и скоро начнет готовиться' : 
                 status === 'cooking' ? 'Наши повара уже готовят ваш заказ' :
                 status === 'on_way' ? 'Курьер уже в пути к вам' :
                 status === 'ready_pickup' ? 'Заказ готов и ждет вас' : 'Следите за статусом'}
              </p>
            </div>
            ...
            <div className="flex gap-3 mt-2">
              ...
              >
                ...
                <span className="text-[10px] font-black text-white uppercase tracking-widest">Позвонить</span>
              </a>
              <button 
                onClick={() => {
                  safeShowAlert("Для отмены заказа, пожалуйста, свяжитесь с нами по телефону.");
                  safeHaptic('warning');
                }}
